...
{
    // Menampilkan form import — terima Kelas10 via Route Model Binding
public function showForm(Request $request)
{
    $kelas10_id = $request->query('kelas10_id');

    if (!$kelas10_id || !is_numeric($kelas10_id)) {
        return redirect()->route('dashboard')->withErrors('ID kelas tidak valid.');
    }

    $kelas10 = Kelas10::where('id', $kelas10_id)->first();
    ...
    public function processImport(Request $request)
    {
        ...
        try {
            ...
            for ($i = $headerRowIndex + 1; $i < count($rows); $i++) {
                ...
                $siswa = IsiKelas10::firstOrCreate(
                    ['modelkelas10s_id' => $kelasId, 'nama' => $nama],
                    ['nisn' => $nisn ?: null]
                );

...
class ImportAbsenkelas11Controller extends Controller
{
    // Menampilkan form import — terima kelas11 via Route Model Binding
public function showForm(Request $request)
{
    $kelas11_id = $request->query('kelas11_id');

    if (!$kelas11_id || !is_numeric($kelas11_id)) {
        return redirect()->route('dashboard')->withErrors('ID kelas tidak valid.');
    }

    $kelas11 = kelas11::where('id', $kelas11_id)->first();

    if (!$kelas11) {
        return redirect()->back()->withErrors('Kelas tidak ditemukan.');
    }

    return view('dashboard.kelas.kelas11.isikelas11.import', compact('kelas11'));
}
    ...
    public function processImport(Request $request)
    {
        ...
        try {
            ...
            for ($i = $headerRowIndex + 1; $i < count($rows); $i++) {
                ...
                $siswa = Isikelas11::firstOrCreate(
                    ['modelkelas11s_id' => $kelasId, 'nama' => $nama],
                    ['nisn' => $nisn ?: null]
                );
                ...
                Absenkelas11::updateOrCreate(
                    [
                        'isi_kelas11_id' => $siswa->id,
                        'bulan' => $bulan,
                        'tahun' => $tahun,
                    ],

...
<div class="container-fluid py-4" style="background-color: #f6ffdf; min-height: 100vh;">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold mb-0">Manajemen Kelas X</h3>
    <div>
      <a href="{{ route('kelas10.create') }}" class="btn btn-success me-2"><i class="bi bi-plus-circle me-1"></i>Add Class</a>
  ...
  <div class="card-container">
    ...
    <div class="card card-custom position-relative">
        ...
        <div class="card-body mt-5" style="overflow-y: auto">
          <h4 class="card-title text-center">Kelas X {{ $variabelkelas10->title }}</h4>
          <p class="card-text text-center">Jumlah siswa: {{ rand(20, 40) }}</p>
          <p class="card-text text-center"> {{ $variabelkelas10->description }} </p>
        </div>
        <div class="card-footer text-center" style="background: inherit; border-color: inherit;">
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('kelas10.destroy', $variabelkelas10->id) }}" method="POST">
                                                <a href="{{ route('kelas10.show', $variabelkelas10->id) }}" class="btn btn-primary btn-sm">SHOW</a>
                                                                                                @csrf
                                                @method('DELETE')
